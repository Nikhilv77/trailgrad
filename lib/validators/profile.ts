...
import { experienceLevelIds, targetRoleIds } from "@/lib/trails/catalog";

export const onboardingTargetRoles = targetRoleIds;

export const profileExperienceLevels = experienceLevelIds;
...
const OnboardingTrailDraftBaseSchema = OnboardingSubmissionBaseSchema.extend({
  trailFocus: z.enum(["job", "learning"]).default("job"),
  targetCompany: z.string().trim().max(120).optional(),
  targetJobTitle: z.string().trim().max(120).optional(),
  applicationDate: z.string().trim().max(20).optional(),
...
export const OnboardingTrailDraftSchema = OnboardingTrailDraftBaseSchema.superRefine((value, context) => {
  if (!value.noDateYet && !value.applicationDate) {
    context.addIssue({
      code: "custom",
      message: "Choose a date or select that you do not have one yet.",
...
export const OnboardingTrailSubmissionSchema = OnboardingTrailDraftBaseSchema.superRefine((value, context) => {
  if (!value.noDateYet && !value.applicationDate) {
    context.addIssue({
      code: "custom",
      message: "Choose a date or select that you do not have one yet.",
...
export const PartialOnboardingSubmissionSchema =
  OnboardingTrailDraftBaseSchema.partial();
